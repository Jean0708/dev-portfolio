...
const heroTrailImages = [
  "/cases/xiaolu-medical.png",
  "/cases/balance-activity.png",
  "/cases/credit-activity.png",
  "/cases/daily-lazcash.png",
  "/cases/dance-plus.png",
  "/assets/profile-jean.png",
];
  ...
  useEffect(() => {
    ...
    const createTrailImage = (
      ...
      const cardSize = Math.min(
        coarsePointer ? bounds.width * 0.34 : bounds.width * 0.14,
        coarsePointer ? 150 : 210,
      );
      if (y > bounds.height - Math.max(150, cardSize)) return;

      const image = document.createElement("img");
      const horizontalJitter = (Math.random() - 0.5) * cardSize * 0.7;
      const startRotation = (Math.random() - 0.5) * 18;
      const endRotation = (Math.random() - 0.5) * 34;
      const landingX = Math.max(
        cardSize / 2,
        Math.min(bounds.width - cardSize / 2, x + deltaX * 2),
      );
      const landingY = bounds.height - cardSize / 2;
      const exitX = Math.max(
        -cardSize,
        Math.min(bounds.width + cardSize, landingX + deltaX * 1.6),
      );

      image.src = heroTrailImages[imageIndex];
      image.alt = "";
      image.setAttribute("aria-hidden", "true");
      image.className = "hero-trail-image";
      image.style.width = `${cardSize}px`;
      hero.appendChild(image);
      activeImages.add(image);

      const firstPhase = image.animate(
        [
          {
            transform: `translate3d(${x + horizontalJitter}px, ${y}px, 0) translate(-50%, -50%) scale(1.28) rotate(${startRotation}deg)`,
          },
          {
            transform: `translate3d(${landingX}px, ${landingY}px, 0) translate(-50%, -50%) scale(.92) rotate(0deg)`,
          },
        ],
        {
          duration: 430,
          easing: "cubic-bezier(.36,.02,.25,1)",
          fill: "forwards",
        },
      );
        ...
        .then(() =>
          image.animate(
            [
              ...
              {
                transform: `translate3d(${exitX}px, ${bounds.height + cardSize * 1.2}px, 0) translate(-50%, -50%) scale(.84) rotate(${endRotation}deg)`,
              },
            ],
            {
              duration: 330,
              easing: "cubic-bezier(.65,0,1,.56)",
              fill: "forwards",
            },
          ).finished,
        )
      ...
      imageIndex = (imageIndex + 1) % heroTrailImages.length;
    };
    ...
    const handlePointerMove = (event: PointerEvent) => {
      ...
      if (distance > bounds.width / (coarsePointer ? 5.5 : 8)) {
        distance = 0;
        createTrailImage(x, y, deltaX);
      }
